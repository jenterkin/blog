...
I stole this idea from some other [smart](https://www.youtube.com/watch?v=HsOKwUwL1bE) [people](https://old.reddit.com/r/proceduralgeneration/comments/ibybmg/uvmapping_pixel_art_for_effortless_content/) when I was searching how to map pixel art onto 2D animations. The solution is actuall pretty simple.
...
For instance, the pixel at `0, 0` will have an `R` value of `0` and a `G` value of `0`. The pixel at `1, 0` will have an `R` value of `1`, a `g` value of `0`, and so on. The following is some GDScript that will generate the UV map and attach it to a `TextureRect`.
